...
def make_layout_uniform():
    ...
    if uav_per_area > 0:
        ...
        my_point = np.random.randint(0, len(total_horizontal_locations_uav), uav_per_area)
        cart_uav[:, 0:2] = total_horizontal_locations_uav[my_point, :]
    ...
    for fixed_point_iteration in range(5):
        ...
        for bs in range(total_base_stations):
            ...
            if len(path_loss_to_ue_temp) == 0 and len(base_station_load_estimation_old_temp) == 0:
                ...
                # interference_to_ues_temp = np.sum(path_loss_to_ue_temp.T * power_load_product, 1)
            ...
            # print(load_per_user.shape, base_station_load_estimation[bs].shape)
